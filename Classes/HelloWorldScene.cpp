...
void HelloWorld::update(float dt){
    bool fire = false;
    auto json = HelloWorld::getBeatJson;
    for (int i = 0; i < 355; i++) {
        if (bValue < json[i] && json[i] < bValue) {
            fire = true;
        }
    }
    bValue += dt;
    ...
    for(int i = 0;i < ids.size();++i){
        ...
        if(bValue < 1){
            ...
            cocos2d::Size size = button->getContentSize();
            Size visibleSize = Director::getInstance()->getVisibleSize();
            ball->setPosition(Vec2(visibleSize.width, position.y));

            auto move  = MoveTo::create(3, Point(position.x ,position.y));
            ball->runAction(move);
            addChild(ball, 1);
        }
    }
